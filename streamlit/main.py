...
def main():
    ...
    ## Page Setup
    tab1, tab2, tab3, tab4 = st.tabs(["How to use?", "Team Colors", "Model Hyperparameters & Detection", "Local Video Player"])
    with tab1:
    ...
    with tab3:
        ...
        st.subheader("Opciones de Redimensionamiento de Salida")
    ...
    with tab4:
        st.header("Reproductor de Video Local")
        st.markdown("Selecciona un directorio para buscar videos y reproducirlos.")

        # Directorio por defecto o el último usado
        default_dir = st.session_state.get('video_dir', '.')
        video_dir = st.text_input("Ruta del directorio de videos", value=default_dir)
        st.session_state['video_dir'] = video_dir

        if os.path.isdir(video_dir):
            video_files_in_dir = [f for f in os.listdir(video_dir) if f.lower().endswith(('.mp4', '.mov', '.avi', '.m4v', '.asf'))]
            ...
            st.write("El directorio especificado no existe o no es válido.")
